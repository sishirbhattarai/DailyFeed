...
$(document).ready(function () {
  ...
  function loadNewsDoc() {
    $.ajax({
      url:
        "https://content.guardianapis.com/search?show-fields=headline,thumbnail&api-key=199bdec0-409f-48d7-a79a-6ff10791c23e",
      method: "GET",
  ...
  searchBtn.on("click", function (event) {
    ...
    $.ajax({
      url:
        ...
        guardianSearch +
        guardianFields +
        "&api-key=199bdec0-409f-48d7-a79a-6ff10791c23e",
      method: "GET",
    }).then(function (response) {
      console.log(response);
  ...
  function getLocation() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(showPosition, function () {
        cityEl.text(
          "Please refresh the page and allow location services for the app to function properly."
        );
      });
    } else {
  ...
  function showPosition(position) {
    ...
    $.ajax({
      url:
        "https://www.mapquestapi.com/geocoding/v1/reverse?key=jHLf4uATR4fijVkLOmrimhIJE79Xp0kx&location=" +
        userLat +
        "," +
        userLong,
      method: "GET",
  ...
  function getWeather() {
    $.ajax({
      url:
        ...
        "&lon=" +
        userLong +
        "&key=b2da4d28d2ce4023b0d33cbef6a3f959",
      method: "GET",
    }).then(function (response) {
      console.log(response);
      console.log("Celsuis temperature: " + response.data[0].temp);
      var tempF = response.data[0].temp * 1.8 + 32;
      ...
      weatherImg.attr(
        "src",
        "./assets/icons/" + response.data[0].weather.icon + ".png"
      );
  ...
  function updateTime() {
    var morningImg =
      "https://i.pinimg.com/originals/6d/df/89/6ddf89a95cc31286387b11c64c1991a8.jpg";
